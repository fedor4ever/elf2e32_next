...
unsigned short Crc(const void* ptr, uint32_t length);

const uint16_t KMaxCheckedUid = 3;
const uint16_t KSizeOf_TUID = sizeof(uint32_t);
constexpr uint16_t KCheckSumBufSize = KMaxCheckedUid * KSizeOf_TUID;
constexpr uint16_t KCrcBufSize = KCheckSumBufSize>>1;

///Checksum every other byte
uint32_t checkSum(const void *ptr)
{

	const uint8_t* begin = (const uint8_t*)ptr;
	const uint8_t* end = begin + KCheckSumBufSize;
	uint8_t buf[KCrcBufSize] = {0};
	uint8_t* tmp = (&buf[0]);
	while (begin < end)
	{
		*tmp++=(*begin);
...
unsigned short Crc(const void* ptr, uint32_t length)
{
	...
	uint32_t crc = 0;
    while(begin < end)
		crc = (crc<<8) ^ crcTab[((crc>>8)^*begin++) & 0xff];
	return crc;
}
...
*/
uint32_t Crc32(const void* ptr, uint32_t aLength)
{
	const uint8_t* p = (const uint8_t *)ptr;
	const uint8_t* q = p + aLength;
	uint32_t crc = 0;
	while (p < q)
