...
elf2e=r" --verbose=-1"
e32=r" --debuggable --smpsafe --verbose=-1"
...
outdated_def_file = e32+caps+defout+elfin+longtail+r" --unfrozen"+r""" --definput="libcryptou_openssl.def" """ + """ --filecrc="libcrypto-2.4.5.SDK.crc;testing_CRCs/dll_outdated.dcrc" """
dso2def = e32+""" --elfinput="libcrypto{000a0000}.dso" """+defout
def2def = e32+defin+ """ --defoutput="tmp\def2def.def" """
elf2baree32 = e32+elfin+""" --output="tmp\elf2baree32.dll" --filecrc=tmp\elf2baree32.dcrc """+implibs+tgttype+linkas+tail
...
outdated_def_file,
outdated_def_file,
("tmp\out.(%02d).def", "tmp\libcrypto-2.4.5.(%02d).dll", "tmp\libcrypto{000a0000}.(0).dso"),
),
...
def SuceededTests(runner, *args):
   ...
   try:
      print tmp[0] %counter
      if "dll_outdated.dcrc" in cmd: # CRC32 validation failed!
         cmd=cmd.replace("libcrypto{000a0000}.(%02d).dso", "libcrypto{000a0000}.(0).dso")
      tmp1=cmd.replace("%02d", str(counter))
      print tmp1
      print "\n"
...
e32+caps+defin+defout+elfin.split("=")[0]+longtail,
"elfin without args failed!",
),
("Test #%d: defin without args",
e32+caps+defin.split("=")[0]+defout+elfin+longtail,
"defin without args failed!",
),
("Test #%d: defin and defout without args",
e32+caps+defin.split("=")[0]+defout.split("=")[0]+elfin+longtail,
"defin and defout without args failed!",
),
("Test #%d: create all except defoutput",
e32+caps+defin+defout.split("=")[0]+elfin+longtail,
"create all except defoutput failed!",
),
("Test #%d: e32image creation without args in params defin defout elfin",
e32+caps+defin.split("=")[0]+defout.split("=")[0]+elfin.split("=")[0]+longtail,
"e32image creation without args in params defin defout elfin failed!",
),
("Test #%d: elf2e32 conversion without other options",
e32+elfin+""" --output="tmp\elf2baree32.dll" """,
"elf2e32 conversion without other options failed!",
) )
